...
PDF_PATH = "Rulebooks/NBA.pdf"

class NBAMetadata(BaseModel):
    league: Literal["NBA"] = "NBA"
...
def _parse_rulebook(raw_text: str) -> list[dict]:
    ...
    for rule in re.split(r'(?=RULE NO\. \d+—)', main_rules):
        if not rule.strip():
            continue
        section_chunks = re.split(r'(?=Section [IVXLCDM]+—)', rule)
...
def load_nba_documents(pdf_path: str = PDF_PATH, max_chars: int = 1500, overlap: int = 200) -> list[dict]:

    print(f"[NBA] Extracting text from {pdf_path}...")
    raw_text = _extract_text(pdf_path)
    chunks = _parse_rulebook(raw_text)
    print(f"[NBA] Parsed {len(chunks)} structural chunks.")
    ...
    print(f"[NBA] Produced {len(documents)} overlapping documents for DB.")
    return documents